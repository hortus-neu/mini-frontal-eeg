...
│   └── cleaned/                   # Preprocessed data (output from preprocess.m)
│       ├── sample_cleaned.set     # EEGLAB main file (metadata + structure)
│       └── sample_cleaned.fdt     # Binary data file associated with the .set file
│
├── figs/                          # Figures generated by the analysis pipeline
│   ├── psd.png                    # Power Spectral Density (PSD) distribution of frontal channels
│   ├── psd_alpha_theta.png        # Alpha / Theta band power comparison across channels
│   ├── faa.png                    # Frontal Alpha Asymmetry (FAA) bar plot
│   └── timefreq_Fz.png            # Time–frequency analysis (ERSP) at Fz channel
│
├── scripts/                       # Core analysis scripts
│   ├── preprocess.m               # Data preprocessing (filtering, bad channel rejection, etc.)
│   ├── psd_bandpower.m            # PSD + band power computation
│   ├── compute_bandpowers_eeglab.m# Helper function calling EEGLAB for bandpower extraction
│   ├── frequency_domain_analysis.m# Stage 3: Frequency-domain analysis (PSD, Alpha/Theta, FAA)
│   ├── timefreq_analysis.m        # Stage 4: Time–frequency analysis (newtimef at Fz)
│   ├── pick_channels_by_labels_for_ffa.m # Channel selection utility (F3, F4, Fz for FAA)
│   └── hello_matlab.m             # Test script (to confirm MATLAB + EEGLAB environment works)
│
├── LICENSE                        # Open-source license for the project
├── README.md                      # Main documentation (background, methods, results, future work)
└── .gitignore                     # Git ignore file (to exclude cache, temp files, etc.)
